...
class GroupService(override val di: DI): CrudService<GroupOutputDto, GroupCreateDto, GroupDao>(di, GroupModel, GroupDao.Companion) {
    ...
    private fun getOne(groupId: Int, authorizedUser: AuthorizedUser) = transaction {
    ...
    fun get(groupId: Int, authorizedUser: AuthorizedUser) = transaction {
        val group = getOne(groupId)
    ...
    fun create(groupCreateDto: GroupCreateClientDto, authorizedUser: AuthorizedUser): GroupOutputClientDto = transaction {
        GroupDao.new {
            name = groupCreateDto.name
            company = CompanyDao[authorizedUser.companyId]
            users = SizedCollection(userService.getByIds(groupCreateDto.users))
        }.toClientOutput()
    }

    fun delete(groupId: Int, authorizedUser: AuthorizedUser): SuccessOutputDto = transaction {
        getOne(groupId, authorizedUser).delete()

        SuccessOutputDto("success", "Group successfully deleted")
    }

    fun update(groupId: Int, groupUpdateDto: GroupCreateClientDto, authorizedUser: AuthorizedUser): GroupOutputClientDto = transaction {
